...
## Parse arguments ----------------------------
parser <- OptionParser()
parser <- add_option(parser, c("-j", "--input_janno"), type = 'character',
                     action = "store", dest = "janno_fn",
                     help = "The input janno file.")
parser <- add_option(parser, c("-e", "--eager_tsv"), type = 'character',
                     action = "store", dest = "eager_tsv_fn",
                     help = "Path to the TSV file used as input for the eager run of your package data.")
parser <- add_option(parser, c("-g", "--general_stats_table"), type = "character",
...
parser <- add_option(parser, c("-c", "--credentials"), type = "character",
...
parser <- add_option(parser, c("-t", "--trust_uncalibrated_dates"), type = "logical",
...
parser <- add_option(parser, c("-p", "--prefer"), type = "character",
...
parser <- add_option(parser, c("-o", "--output_janno"), type = "character",
