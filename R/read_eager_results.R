  ...
  tsv_data <- tsv_data %>%
    dplyr::mutate(
      ...
      Seq_Type=dplyr::case_when(
        ...
        !is.na(.data$R1) ~ basename(.data$R1) %>% substr(.,14,15) %>% format_for_poseidon(., "Capture_Type"),
        ##  Else, check the BAM input (must be there if no R1 is provided)
        !is.na(.data$BAM) ~ basename(.data$BAM) %>% substr(.,14,15) %>% format_for_poseidon(., "Capture_Type"),
...
read_eager_stats_table <- function(general_stats_fn, tsv_data) {
  ...
  general_stats <- readr::read_tsv(general_stats_fn, na = c("", "N/A", "NA")) %>% dplyr::filter(.data$Sample %in% pull_samples) %>%
    dplyr::select(
      .data$Sample,
  ...
  contamination_per_sample <- general_stats  %>%
    tidyr::separate(.data$Sample, sep="\\.", into=c("Sample","Library"), fill="right", extra="merge") %>%
    dplyr::filter(!is.na(.data$Library)) %>% ## Keep only Library Entries
    dplyr::group_by(.data$Sample) %>%
    dplyr::summarise(
